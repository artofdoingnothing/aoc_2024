...
def parse_file():
    with open('./day_13/real_data.txt', 'r') as reader:
        ...
        for i in range(0, len(lines), 4):
            ...
            a_move = (int(a_match.group(1)), int(a_match.group(2)))
            b_move = (int(b_match.group(1)), int(b_match.group(2)))
            prize_position = (int(prize_match.group(1)), int(prize_match.group(2)))
        
            prize_info.append((a_move, b_move, prize_position))
